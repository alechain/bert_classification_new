...
class NewsSpider(CrawlSpider):
    ...
    def start_requests(self):
        urls_file_path = '/Users/achain/Documents/github/web-mining/pages/economia_urls.txt'
        with open(urls_file_path, 'r') as urls_file:
            for url in urls_file:
                yield scrapy.Request(url=url.strip(), callback=self.parse_response)

    def parse_response(self, response):
        article_content = response.text
        # Get the filename from the URL
        filename = response.url.split('/')[-1] + '.html'  # Add .html extension
        save_path = '/Users/achain/Documents/github/web-mining/pages/eco_content'
        ...
        with open(os.path.join(save_path, filename), 'w', encoding='utf-8') as file:
            file.write(article_content)  # Save the article content
